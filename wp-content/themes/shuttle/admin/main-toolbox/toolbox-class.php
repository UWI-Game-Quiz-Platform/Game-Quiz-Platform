 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'shuttle_toolbox_section' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class shuttle_toolbox_section {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The shuttle_toolbox_section class version.
		 */
		...
		/**
		 * The single instance of shuttle_toolbox_section
		 *
		 * @var shuttle_toolbox_section $instance The shuttle_toolbox_section instance.
		 */
		private static $instance;

		/**
		 * The Main shuttle_toolbox_section instance.
		 *
		 * We make sure that only one instance of shuttle_toolbox_section exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof shuttle_toolbox_section ) ) {
				self::$instance = new shuttle_toolbox_section;
		...
		public function setup_actions() {
			...
			add_action( 'admin_menu', array( $this, 'register' ), 0 );
		...
		function body_class( $classes ) {
			...
			$classes = $classes . ' theme-shuttlethemes';
			$classes = $classes . ' theme-parent-' . strtolower($this->theme_name);
			$classes = $classes . ' theme-child-' . $this->theme_name_base;
...
//				);
				add_submenu_page(
					...
					esc_html__( 'Upgrade Now', strtolower( $theme_name ) ),
					'activate_plugins',
					esc_url( 'https://shuttlethemes.com/features/?utm_campaign=upgrade&utm_medium=sidebar&utm_source=WordPress' )
				);
			    // add_submenu_page(
				// 	'shuttle-setup',
...
//					}
				...
				echo	'<div id="shuttle-promotion-field-header">';
				...
				if ( ! empty( $this->tabs ) ) {
					...
					foreach ( $this->tabs as $tab_key => $tab_name ) {
						...
						echo '<a href="' . esc_url( admin_url( 'admin.php?page=shuttle-setup' ) ) . '&tab=' . $tab_key . '" class="nav-tab ' . ( $active_tab == $tab_key ? 'nav-tab-active' : '' ) . '" role="tab" data-toggle="tab">';
		...
		public function page_start() {
			...
			if ( ! empty( $this->config['page_start'] ) ) {
				...
				if ( ! empty( $get_started ) ) {
					...
					echo '<div id="shuttle-get-started" class="feature-section three-col">';
		...
		public function page_docs() {
			echo '<div id="shuttle-documentation" class="feature-section one-col">';
			...
			if ( ! empty( $this->config['page_docs'] ) ) {
				...
				if ( ! empty( $documentation_steps ) ) {
					...
					echo '<div class="shuttle-documentation-info">';
		...
		public function page_support() {
			echo '<div id="shuttle-support-content" class="feature-section three-col">';
		...
		public function page_free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					echo '<div id="shuttle-free-pro" class="feature-section">';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						echo '<tr class="shuttle-free-pro-button">';
						echo '<td></td>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'shuttle-global-css', get_template_directory_uri() . '/admin/main-toolbox/assets/css/toolbox-backend-global.css' );

			// enqueue css files
			if ( 'themes.php' === $pagenow || strpos($hook_suffix, 'shuttle') !== false ) {
				wp_enqueue_style( 'shuttle-page-css', get_template_directory_uri() . '/admin/main-toolbox/assets/css/toolbox-backend.css' );
			}
